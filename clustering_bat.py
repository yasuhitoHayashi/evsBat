...
# 極性が0のデータのみを使用
data_filtered = data[(data['polarity'] == 0)].copy()# & (data['y'] >= 100) & (data['y'] <= 400)].copy()
...
for time_bin in progressbar.progressbar(data_filtered['time_bin'].unique(), prefix='クラスタリング進行中: '):
    ...
    # DBSCANによるクラスタリング
    db = DBSCAN(eps=25, min_samples=5).fit(bin_data)
    labels = db.labels_
...
pkl_output_file = 'clustering_kiku_2860.pkl'
