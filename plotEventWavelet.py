...
def process_pickle_file(particle_output_file):
    ...
    for i, peak in enumerate(peaks):
        ...
        color = 'red' if i == max_height_index else 'black'
        plt.scatter(freq, amplitude, color=color, marker='v')
        plt.text(freq, amplitude + 2, f'{freq:.2f} Hz', fontsize=10, color=color, ha='center')
    ...
    coef, freqs = pywt.cwt(event_counts, scales, 'morl', sampling_period=time_bin_size/1000.0)
    plt.subplot(3, 1, 3)
    plt.imshow(np.abs(coef), extent=[min_time, max_time, freqs[-1], freqs[0]], aspect='auto', cmap='jet')
    plt.colorbar(label='Magnitude')
    plt.yscale('log')
    plt.xlabel('Time (milliseconds)')
    plt.ylabel('Frequency (Hz)')
    plt.title('Wavelet Transform (Morlet) - Event Counts')

    # PNGファイルとして保存
    output_image_file = os.path.join(fft_results_dir, f'{base_filename}_fft_wavelet_analysis.png')
    plt.tight_layout()
    plt.savefig(output_image_file)
    plt.close()
    print(f"FFT and Wavelet plot saved to {output_image_file}")
